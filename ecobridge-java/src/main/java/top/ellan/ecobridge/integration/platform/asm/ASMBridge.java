 ...
 * ASMBridge
 *
 * Replace UltimateShop economy price calculation with EcoBridge snapshot pricing.
 */
public class ASMBridge {
    ...
    public static ObjectPrices redirectPrice(ObjectItem item, ObjectPrices originalPrices, boolean isBuy) {
        try {
            ...
            if (replaced <= 0) {
                LogUtil.warnOnce("asm-no-economy-price",
                        "ASM price redirect active, but no economy price entry was found for " + shopId + "." + productId);
            }
            return originalPrices;
    ...
    private static int overrideEconomyEntries(ObjectPrices prices, double newValue) throws IllegalAccessException {
        ...
        for (Field f : getAllFields(target.getClass())) {
            try {
                ...
                if (type == double.class) {
                    ...
                    if (name.contains("amount") || name.contains("value") || name.contains("price")
                            || name.contains("formula") || name.contains("text") || name.contains("parse")) {
