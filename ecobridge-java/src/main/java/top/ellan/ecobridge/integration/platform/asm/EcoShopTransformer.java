 ...
 *
 * <p>修复记录: 1. [CRITICAL] 移除了 visitField 注入逻辑，解决了 "class redefinition failed" 崩溃。 2. [Logic]
 * LimitAdapter 改为后置修正，支持 Shadow Mode 观察原插件逻辑。 3. [Safety] 使用 ASM 指令扫描替代字段标记进行幂等性检查。
 */
public class EcoShopTransformer implements ClassFileTransformer {
  ...
  // 桥接类的内部名，用于检测是否已注入
  private static final String BRIDGE_CLASS_INTERNAL =
      "top/ellan/ecobridge/integration/platform/asm/ASMBridge";
          ...
          new ClassVisitor(Opcodes.ASM9, cw) {
            ...
            public MethodVisitor visitMethod(
              ...
              if ((name.equals("getBuyPrice") || name.equals("getSellPrice"))
                  && descriptor.endsWith(
                      "Lcn/superiormc/ultimateshop/objects/items/prices/ObjectPrices;")) {
  ...
  /** [修复] 通过扫描方法调用指令来检测是否已注入 替代了之前的字段检测，避免了 Schema Change 异常 */
      ...
      cr.accept(
          new ClassVisitor(Opcodes.ASM9) {
            ...
            public MethodVisitor visitMethod(
                int access, String name, String descriptor, String signature, String[] exceptions) {
              // 只需扫描几个关键方法
              if (hooked.get() || (!name.equals("getBuyPrice") && !name.endsWith("Limit")))
                return null;
  ...
  /** 价格拦截适配器 逻辑: 原方法返回 -> 栈顶是 ObjectPrices -> 调用 redirectPrice -> 返回修改后的 ObjectPrices */
  private static class PriceAdapter extends AdviceAdapter {
    ...
    protected void onMethodExit(int opcode) {
      if (opcode == ARETURN) { // 拦截引用类型返回
        ...
        invokeStatic(
            Type.getType("L" + BRIDGE_CLASS_INTERNAL + ";"),
            new Method(
                "redirectPrice",
  ...
  /** 限额拦截适配器 逻辑: 原方法返回 -> 栈顶是 int (limit) -> 调用 adjustLimit -> 返回修改后的 int */
    ...
    protected void onMethodExit(int opcode) {
      if (opcode == IRETURN) { // 拦截 int 返回
        ...
        invokeStatic(
            Type.getType("L" + BRIDGE_CLASS_INTERNAL + ";"),
            new Method(
                "adjustLimit", "(Lcn/superiormc/ultimateshop/objects/buttons/ObjectItem;IZ)I"));
      }
    }
  }
}
