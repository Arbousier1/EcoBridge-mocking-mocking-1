...
/**
 * EcoBridge 占位符扩展 - 高性能注册表版 (v1.6.0) * 核心重构： 1. 引入 Map 路由表取代冗长的 switch/if-else 结构。 2. 保持 VarHandle
 * 的高安全性内存访问逻辑。 3. 支持前缀动态变量路由。
 */
public class EcoPlaceholderExpansion extends PlaceholderExpansion {
  ...
  // 静态变量处理器 (精确匹配)
  private final Map<String, BiFunction<OfflinePlayer, String, String>> staticHandlers =
      new HashMap<>();
  ...
  private void initializeRegistry() {
    ...
    staticHandlers.put(
        "stability",
        (p, args) -> String.format("%.2f", EconomyManager.getInstance().getMarketStability()));
    staticHandlers.put(
        "market_heat",
        (p, args) -> String.format("%.1f", EconomyManager.getInstance().getMarketHeat()));
    staticHandlers.put(
        "eco_saturation",
        (p, args) ->
            String.format("%.2f%%", EconomyManager.getInstance().getEcoSaturation() * 100));
    staticHandlers.put("is_holiday", (p, args) -> HolidayManager.isTodayHoliday() ? "是" : "否");
    staticHandlers.put(
        "holiday_mult",
        (p, args) -> String.format("%.1fx", HolidayManager.getHolidayEpsilonFactor()));

    // --- 2. PID 内核与 Native 状态 ---
    staticHandlers.put("native_status", (p, args) -> NativeBridge.isLoaded() ? "已就绪" : "未加载");
  ...
  public @Nullable String onRequest(OfflinePlayer player, @NotNull String params) {
    ...
    if (params.startsWith("buy_price_")) {
      ...
      return PricingManager.getInstance() != null
          ? String.format("%.2f", PricingManager.getInstance().calculateBuyPrice(productId))
          : "0.00";
    }
    ...
    if (params.startsWith("sell_price_")) {
      String productId = params.substring(11);
      return PricingManager.getInstance() != null
          ? String.format("%.2f", PricingManager.getInstance().calculateSellPrice(productId))
          : "0.00";
    }
