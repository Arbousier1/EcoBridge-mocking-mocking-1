...
/**
 * 智能转账指令 (TransferCommand v0.6.3 - UX Polish) 职责：指令解析、权限验证、Tab 补全，并驱动 TransferManager 进入风控审计流。
...
public class TransferCommand implements TabExecutor {
  ...
  public boolean onCommand(
    ...
    if (!player.hasPermission(PERMISSION)) {
      player.sendMessage(
          EcoBridge.getMiniMessage().deserialize("<red>权限不足！ <gray>您需要: <yellow>" + PERMISSION));
      return true;
    }
    ...
    if (target == null || !target.isOnline()) {
      player.sendMessage(
          EcoBridge.getMiniMessage()
              .deserialize("<red>错误: <gray>目标玩家 <yellow>" + args[0] + " <gray>不在线。"));
      return true;
    }
    ...
    try {
      ...
      if (finalAmount < 0.01) {
        player.sendMessage(
            EcoBridge.getMiniMessage().deserialize("<red>金额异常: <gray>最小转账单位为 <yellow>0.01"));
        return true;
      }
    ...
    player.sendMessage(
        EcoBridge.getMiniMessage()
            .deserialize("<gray>[<blue>EcoBridge</blue>] <italic>正在启动智能审计，请稍候...</italic>"));
